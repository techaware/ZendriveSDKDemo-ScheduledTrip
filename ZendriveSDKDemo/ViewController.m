...
static NSString * kZendriveSDKKeyString = @"your-idk-key";

@interface ViewController () <ZendriveDelegateProtocol, UITableViewDelegate, UITableViewDataSource, ScheduleStartDelegate>
...
//  invalidate timer is it's already running
    [self.timer invalidate];
    
//  navigate to slider view to scedule a manual trip  
    ScheduleStartController *schedule = [[ScheduleStartController alloc] init];
...
//------------------------------------------------------------------------------
#pragma mark - ScheduleStartDelegate
//------------------------------------------------------------------------------
...
//  invalidate timer to start fresh
    [self.timer invalidate];
    
//    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:60 target:self selector:@selector(myTicker) repeats:YES];
...
-(void)timerFired {
    ...
    self.driveStatusLabel.text = [NSString stringWithFormat:@"Your trip will start in %d Seconds",self.sliderValue];
    if(self.sliderValue==0){
        [self.timer invalidate];
        [Zendrive startDrive:@"your-tracking-id-here"];
    }
